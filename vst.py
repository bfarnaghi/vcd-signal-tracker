...
def monitor_signals(vcd, signals, start_time, end_time):
    
    monitored_data = {signal: [] for signal in signals}
    for signal in signals:
        sig_data = vcd[signal].tv
...
def generate_vcd_header(monitored_data):
    ...
    header.append("$version Generated by VCDPARSER $end\n")
...
def generate_vcd_files_with_monitored_data(start_time,end_time,num_cycles, monitored_data, output_folder):
    ...
    for cycle in range(num_cycles):
        vcd_file_path = os.path.join(output_folder, f"cycle_{cycle}.vcd")
        with open(vcd_file_path, 'a') as f:
...
if __name__ == "__main__":
    ...
    monitored_data = monitor_signals(vcd, vcd.get_signals(), start_time, end_time)
